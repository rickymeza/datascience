...
pollsApril <- brexit_polls %>% filter(month(brexit_polls$startdate) == 4)
...
pollsEnding <- brexit_polls %>% filter(round_date(enddate,unit = "week") == dateComp)

nrow(pollsEnding) 

dayEnding <- weekdays(brexit_polls$enddate)
...
noStopWords2 <- noStopWords %>% filter(!str_detect(word, "\\d+"))
nrow(noStopWords2)

noStopWords2 %>% count(word, sort=TRUE) %>% filter(n>100)

afinn <- get_sentiments("afinn")
afinn_sentiments <- noStopWords2 %>% inner_join(afinn, by = "word")
